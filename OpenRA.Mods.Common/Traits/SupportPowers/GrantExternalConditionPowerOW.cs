...
namespace OpenRA.Mods.Common.Traits
{
	public class GrantExternalConditionPowerOWInfo : SupportPowerInfo
	{
		...
		public override object Create(ActorInitializer init) { return new GrantExternalConditionPowerOW(init.Self, this); }
	}

	public class GrantExternalConditionPowerOW : SupportPower
	{
		readonly GrantExternalConditionPowerOWInfo info;
		readonly char[] footprint;

		public GrantExternalConditionPowerOW(Actor self, GrantExternalConditionPowerOWInfo info)
			: base(self, info)
		{
			this.info = info;
		...
		public override void Activate(Actor self, Order order, SupportPowerManager manager)
		{
			...
			foreach (var a in UnitsInRange(self.World.Map.CellContaining(order.Target.CenterPosition)))
				a.TraitsImplementing<ExternalCondition>()
					.FirstOrDefault(t => t.Info.Condition == info.NegativeCondition && t.CanGrantCondition(self))
					?.GrantCondition(a, self, info.Duration);
		}
		...
		sealed class SelectConditionTarget : OrderGenerator
		{
			...
			readonly GrantExternalConditionPowerOW power;
			readonly char[] footprint;
			...
			public SelectConditionTarget(World world, string order, SupportPowerManager manager, GrantExternalConditionPowerOW power)
				: base(world)
			{
			...
			protected override IEnumerable<IRenderable> RenderAnnotations(WorldRenderer wr, World world)
			{
				...
				foreach (var unit in power.UnitsInRange(xy))
				{
					...
					var killsself = unit.TraitsImplementing<ExternalCondition>()
					.Any(t => t.Info.Condition == power.info.NegativeCondition && t.CanGrantCondition(power.Self));
					if (decorations != null && !killsself)
