...
{
	sealed class KillsSelfOnParentDeathInfo : ConditionalTraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new KillsSelfOnParentDeath(init.Self, this, init); }
	}

	sealed class KillsSelfOnParentDeath : ConditionalTrait<KillsSelfOnParentDeathInfo>, INotifyAddedToWorld, ITick
	{
		int lifetime;

		Actor parent;

		readonly bool hasParent = true;

		public KillsSelfOnParentDeath(Actor self, KillsSelfOnParentDeathInfo info, ActorInitializer init)
			: base(info)
		{
		...
		{
			if (!self.IsInWorld || self.IsDead || IsTraitDisabled || (hasParent && parent.IsInWorld))
				return;
