...
{
	[Desc("Play the Kill voice of this actor when eliminating enemies. This version checks for PlayerRelationship")]
	public class AnnounceOnKillOWInfo : TraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new AnnounceOnKillOW(this); }
	}

	public class AnnounceOnKillOW : INotifyAppliedDamage
	{
		readonly AnnounceOnKillOWInfo info;

		long lastAnnounce;

		public AnnounceOnKillOW(AnnounceOnKillOWInfo info)
		{
			this.info = info;
		...
		void INotifyAppliedDamage.AppliedDamage(Actor self, Actor damaged, AttackInfo e)
		{
			...
			{
				if (!info.ValidStances.HasRelationship(e.Attacker.Owner.RelationshipWith(damaged.Owner)))
					return;
