...
namespace OpenRA.Mods.Cnc.Traits
{
	...
	sealed class AttackTeslaOWInfo : AttackBaseInfo
	{
		...
		public override object Create(ActorInitializer init) { return new AttackTeslaOW(init.Self, this); }
	}

	sealed class AttackTeslaOW : AttackBase, ITick, INotifyAttack
	{
		readonly AttackTeslaOWInfo info;
		...
		public AttackTeslaOW(Actor self, AttackTeslaOWInfo info)
			: base(self, info)
		{
			this.info = info;
		...
		void INotifyAttack.Attacking(Actor self, in Target target, Armament a, Barrel barrel)
		{
			...
			timeToRecharge = Util.ApplyPercentageModifiers(info.ReloadDelay, modifiers);
		}
		...
		{
			return new ChargeAttackOW(this, newTarget, forceAttack, targetLineColor);
		}

		sealed class ChargeAttackOW : Activity, IActivityNotifyStanceChanged
		{
			readonly AttackTeslaOW attack;
			readonly Target target;
			readonly bool forceAttack;
			readonly Color? targetLineColor;

			public ChargeAttackOW(AttackTeslaOW attack, in Target target, bool forceAttack, Color? targetLineColor = null)
			{
				this.attack = attack;
				this.target = target;
			...
			public override bool Tick(Actor self)
			{
				...
				QueueChild(new ChargeFireOW(attack, target));
				return false;
			}
		...
		sealed class ChargeFireOW : Activity
		{
			readonly AttackTeslaOW attack;
			readonly Target target;

			public ChargeFireOW(AttackTeslaOW attack, in Target target)
			{
				this.attack = attack;
				this.target = target;
			}
