...
{
	public class FallToEarthOW : Activity
	{
		...
		public FallToEarthOW(Actor self, FallsToEarthOWInfo info)
		{
			this.info = info;
		...
		public override bool Tick(Actor self)
		{
			if (self.World.Map.DistanceAboveTerrain(self.CenterPosition).Length <= 0)
			{
				...
				if (info.FiredByOwner)
					info.ExplosionWeapon?.Impact(Target.FromPos(self.CenterPosition), self.Owner.PlayerActor);
				else
